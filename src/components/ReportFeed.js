...
const generateMockData = (id) => {
    // Deterministic generation based on ID so it looks consistent
    const types = ["Theft", "Violence", "Cybercrime", "Harassment", "Corruption"];
    const locs = ["Andheri East, Mumbai", "Connaught Place, Delhi", "Koramangala, Bangalore", "Salt Lake, Kolkata", "Hitech City, Hyderabad"];
    const urgencies = ["High", "Medium", "Low"];
    
    const numId = Number(id);
    return {
        crimeType: types[numId % types.length],
        urgency: urgencies[numId % urgencies.length],
        location: locs[numId % locs.length],
        incidentDateTime: new Date().toISOString(),
        description: `(DEMO DECRYPTED) Detailed report for Case #${id}. Witness observed suspicious activity involving three individuals. Evidence suggests coordination. Requesting immediate patrol unit verification. This data was securely decrypted from the blockchain ledger.`,
        mediaHash: null // Keep null for demo stability unless you have a reliable hash
    };
};
...
export default function ReportFeed() {
  ...
  // 2. Load Statuses
  useEffect(() => {
    const saved = localStorage.getItem('veil_admin_statuses');
    if (saved) setReportStatuses(JSON.parse(saved));
  }, []);
  ...
  // 3. SIMULATED DECRYPT FUNCTION
  const handleDecrypt = async (id, reportCID) => {
    setLoadingId(id);
    // Simulate Network Delay for realism
    setTimeout(() => {
        const mockData = generateMockData(id);
        setDecryptedData(prev => ({ ...prev, [id]: mockData }));
        setLoadingId(null);
    }, 1500); // 1.5s delay
  };

  // 4. DECRYPT ALL FUNCTION
  const handleDecryptAll = () => {
    ...
    setTimeout(() => {
        ...
        reports.forEach(report => {
            ...
            if (!decryptedData[id]) {
                newData[id] = generateMockData(id);
            }
        });
  ...
  const solved = Object.values(reportStatuses).filter(s => s === 'SOLVED').length;
  const spam = Object.values(reportStatuses).filter(s => s === 'SPAM').length;
  const pending = total - solved - spam;

  if (isLoading) return <div className="text-white mt-10 animate-pulse">Syncing Admin Ledger...</div>;
  if (isError) return <div className="text-red-500 mt-10">Access Denied: Connect Admin Wallet</div>;
  ...
  return (
    <div className="w-full max-w-5xl mt-8 mb-20">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-neutral-900/50 p-4 rounded-xl border border-neutral-800">
            <p className="text-xs text-orange-500 uppercase font-bold">Pending Action</p>
            <p className="text-2xl font-bold text-orange-400">{pending}</p>
        </div>
        <div className="bg-neutral-900/50 p-4 rounded-xl border border-neutral-800">
            <p className="text-xs text-green-500 uppercase font-bold">Cases Solved</p>
            <p className="text-2xl font-bold text-green-400">{solved}</p>
        </div>
        <div className="bg-neutral-900/50 p-4 rounded-xl border border-neutral-800">
            <p className="text-xs text-red-500 uppercase font-bold">Flagged Spam</p>
            <p className="text-2xl font-bold text-red-400">{spam}</p>
        </div>
      </div>

      {/* --- CONTROLS BAR (Filter + Decrypt All) --- */}
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
          
          {/* Filters */}
          <div className="flex flex-wrap gap-2 p-2 bg-neutral-900 rounded-xl border border-neutral-800 overflow-x-auto w-full md:w-auto">
          ...
          <button
            ...
            disabled={isDecryptingAll || reports?.length === 0}
      ...
      <div className="grid gap-6">
        {displayReports.map((report, index) => {
          ...
          return (
            <div key={index} className={`relative p-6 rounded-xl border transition-all duration-300 ${
                isSolved ? 'bg-green-900/10 border-green-900/30' : 
                isSpam ? 'bg-red-900/10 border-red-900/30 opacity-60' : 
                'bg-neutral-800 border-neutral-700 shadow-xl'
            }`}>
                {/* Badges */}
                ...
                <div className="flex justify-between items-start mb-4">
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-gray-600 font-mono">CID: {report.reportCID.substring(0, 12)}...</span>
                        <span className="text-[10px] text-gray-600 font-mono">{new Date(Number(report.timestamp) * 1000).toLocaleString()}</span>
                    </div>
                </div>

                {data ? (
                  // --- DECRYPTED CONTENT (MOCK) ---
                  <div className="animate-in fade-in">
                    <div className="flex gap-2 mb-4">
                        ...
                        <span className={`text-xs px-2 py-1 rounded font-bold border ${
                            data.urgency === 'High' ? 'text-red-400 border-red-900 bg-red-900/20' : 
                            data.urgency === 'Medium' ? 'text-orange-400 border-orange-900 bg-orange-900/20' : 
                    ...
                    <div className="grid md:grid-cols-2 gap-4 mb-4">
                         ...
                         <div className="bg-neutral-900/50 p-3 rounded border border-neutral-800">
                            ...
                            <p className="text-sm text-gray-300">{new Date(data.incidentDateTime).toLocaleString()}</p>
                         </div>
                    </div>
                    ...
                    <div className="w-full h-40 bg-neutral-900 rounded overflow-hidden border border-neutral-700 mb-4 relative">
                        <iframe
                            ...
                            src={`https://maps.google.com/maps?q=${encodeURIComponent(data.location)}&t=&z=14&ie=UTF8&iwloc=&output=embed`}
                    ...
                    <div className="flex gap-3 pt-4 border-t border-neutral-700">
                        <button 
                            onClick={() => updateStatus(reportId, 'SOLVED')}
                            disabled={isSolved}
                            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg font-bold transition-all ${
                                isSolved ? 'bg-green-600 text-white' : 'bg-neutral-700 hover:bg-green-600 text-white'
                            }`}
                        >
                            <CheckIcon /> Approve
                        </button>
                        <button 
                            onClick={() => updateStatus(reportId, 'SPAM')}
                            disabled={isSpam}
                            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg font-bold transition-all ${
                                isSpam ? 'bg-neutral-800 text-red-500 border border-red-900' : 'bg-neutral-900 border border-neutral-700 hover:border-red-500 hover:text-red-500'
                            }`}
                        >
                            <XIcon /> Spam
                        </button>
                  ...
                  // --- LOCKED STATE ---
                  <button 
                    onClick={() => handleDecrypt(reportId, report.reportCID)}
                    disabled={loadingId === reportId || isDecryptingAll}
                    className="w-full h-40 bg-neutral-900/50 border border-dashed border-neutral-700 rounded-lg flex flex-col items-center justify-center gap-3 hover:bg-neutral-900 transition-all group"
                  >
                    {loadingId === reportId || isDecryptingAll ? (
                        <span className="text-red-500 font-mono animate-pulse">Decrypting...</span>
                    ) : (
                        <>
                            <span className="text-3xl grayscale group-hover:grayscale-0">🔐</span>
                            <div className="text-center">
                                <p className="text-gray-400 font-mono text-sm">Encrypted Data</p>
                                <p className="text-red-500 text-xs mt-1 opacity-0 group-hover:opacity-100">Click to Decrypt</p>
                            </div>
                        </>
                    )}
                  </button>
